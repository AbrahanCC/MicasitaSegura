...
<body>
  ...
  <script>
    ...
    async function validate(token, dir) {
      ...
      return await resp.json(); // {valid, reason, used_count?, type?}
    }
      ...
      try {
        ...
        const onScanSuccess = async (decodedText) => {
          ...
          try {
            ...
            if (j.valid) {
              const info =
                (j.type === 'POR_INTENTOS' && j.used_count != null)
                  ? ` (usos consumidos: ${j.used_count})`
                  : '';
              const tipoTxt = j.type ? ` [${j.type}]` : '';
              setStatus("VÁLIDO — apertura en curso…" + info + tipoTxt, true);
